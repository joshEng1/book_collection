...
RSpec.describe "Book CRUD", type: :request do
  ...
  it "provides all five book pages and a home link" do
    [ books_path, new_book_path, book_path(book),
      edit_book_path(book), delete_book_path(book) ].each do |path|
      get path
  ...
  it "prepopulates fields and uses three date dropdowns" do
    get edit_book_path(book)
    document = Nokogiri::HTML(response.body)
    title = document.at_css('input[name="book[title]"]')
    author = document.at_css('input[name="book[author]"]')
    dates = document.css('select[name^="book[published_date"]')
    expect(title["value"]).to eq("Dune")
    expect(author["value"]).to eq("Frank Herbert")
    expect(dates.size).to eq(3)
    expect(document.css("select option[selected]").map do |option|
      option["value"]
    end).to eq(%w[1965 8 1])
  end

  it "updates all attributes and shows a home-page notice" do
    patch book_path(book),
      params: { book: { title: "Foundation", author: "Isaac Asimov",
                        price: "9.50",
                        published_date: "1951-06-01" } }
    expect(response).to redirect_to(root_path)
    expect(book.reload.attributes.slice("title", "author", "price",
      "published_date")).to eq(
        "title" => "Foundation", "author" => "Isaac Asimov",
        "price" => BigDecimal("9.50"),
        "published_date" => Date.new(1951, 6, 1)
    )
    follow_redirect!
  ...
  it "does not change a record on an invalid update" do
    patch book_path(book),
      params: { book: { title: "", author: "Changed" } }
  ...
  it "rejects an incomplete publication date" do
    expect do
      post books_path,
        params: { book: { title: "Incomplete date",
                          "published_date(1i)" => "1965",
                          "published_date(2i)" => "",
                          "published_date(3i)" => "1" } }
    end.not_to change(Book, :count)
    expect(response).to have_http_status(:unprocessable_content)
  ...
  it "requires DELETE and shows a home-page deletion notice" do
  ...
  it "returns all book attributes through the existing JSON API" do
    ...
    expect(response.parsed_body).to include("title" => "Dune",
      "author" => "Frank Herbert", "price" => "12.99",
