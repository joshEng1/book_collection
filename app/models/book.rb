class Book < ApplicationRecord
  ...
  # Rails otherwise supplies defaults for missing date-select components.
  def published_date=(value)
    @invalid_published_date = false
    if value.is_a?(Hash)
      parts = value.values_at(1, 2, 3)
      @invalid_published_date = !parts.all? { |part| part.is_a?(Integer) } || !Date.valid_date?(*parts)
  ...
  private
    ...
    def published_date_must_be_complete
      errors.add(:published_date, "must be a complete, valid date") if @invalid_published_date
    end
end
